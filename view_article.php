...
{
	extract($_GET);
	if(strpos($share_link, "view_article.php?b_id=")!==false)
    {
        header('HTTP/1.1 301 Moved Permanently');
        header('Location:'.ROOT_URL_FRONT.'article/'.$b_id.'.html');
        exit();
    }

    if(array_key_exists("user_role", $_SESSION))
    {
        $sqlb="SELECT * FROM articles WHERE url_id='$b_id';";
        $qb= $conn->query($sqlb);
        $new_blog=$qb->fetchAll(PDO::FETCH_ASSOC);
    }
    else
    {
    	$sqlb="SELECT * FROM articles WHERE publish='1' AND url_id='$b_id';";
    	$qb= $conn->query($sqlb);
    	$new_blog=$qb->fetchAll(PDO::FETCH_ASSOC);

        $sqlup="UPDATE articles SET view_count=(view_count+1) WHERE publish='1' AND id='".$new_blog[0]['id']."';";
        $qup= $conn->prepare($sqlup);
        $qup->execute();
    }

    $sqlc="SELECT c.*,ct.cust_name FROM comments c LEFT JOIN customers ct ON  c.cust_id=ct.id WHERE c.nblog_id='".$new_blog[0]['id']."';";
    $qc= $conn->query($sqlc);
...
if (!empty($_COOKIE["cust_hash"])) {
    $likes="SELECT * FROM like_section WHERE cust_hash='".$_COOKIE['cust_hash']."' AND article_id='".$new_blog[0]['id']."';";
    $likes=$conn->query($likes);
    $likes=$likes->fetchAll(PDO::FETCH_ASSOC);
}

$page_name=$new_blog[0]['b_name'];
$desc= $new_blog[0]['b_subhead'];
?>
...
<script type="application/ld+json">
{
    ...
    "mainEntityOfPage":
    {
        ...
        "headline":"<?=$new_blog[0]['b_name']; ?>",
        "datePublished":"<?=date("c",strtotime($new_blog[0]['b_date'])); ?>",
        "dateModified":"<?=date("c",strtotime($new_blog[0]['updated_at'])); ?>",
        "articleBody":"<?=trim(preg_replace('/\s\s+/', ' ', str_replace("\n", " ", strip_tags($new_blog[0]['b_content']))));?>",
        "keywords":"<?=$new_blog[0]['tags'];?>",
        "url":"<?=(isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";?>",
        "description":"<?php if (!empty($new_blog[0]['b_subhead'])) { echo $new_blog[0]['b_subhead']; }else{ echo trim(preg_replace('/\s\s+/', ' ', str_replace("\n", " ", strip_tags($new_blog[0]['b_content'])))); } ?>",
        "author":
        [
            {
                "@type":"Person",
                "name":"<?php if (!empty($new_blog[0]['b_author'])) { echo $new_blog[0]['b_author']; }else{ echo "Rachna Tyagi"; } ?>"
            },
            {
                "@type":"Thing",
                "name":"Turn of Speed",
        ...
        [
            {
                "@type":"ImageObject",
                "url":"<?=ROOT_URL.$new_blog[0]['cover_photo']; ?>"
            }
        ]
    }
}
...
<?php
if($new_blog[0]['is_cover_story']=='1'){ ?>
    <style>
			...
			<div class="container">
				<div class="row hentry-single">
                    <div class="col-sm-12 col-md-12">
                        <h3 class="hentry-title"> <?=$new_blog[0]['b_name']; ?></h3>
                        <span class="sep block mt-10 mb-10"></span>
                        <h6 class="hentry-title"><i><?=$new_blog[0]['b_subhead']; ?></i></h6>
                        <br>
    					<div class="col-xs-12 col-md-8">
    						<article>
                              <?php if(!empty($new_blog)){ foreach($new_blog as $blg){ ?>
                               <div class="hentry-meta flex flex-middle disable-flex-xs fw700 mt-15">
                                ...
                                <?php } ?>
                                    <span class="comments float l-review <?php if(!empty($likes)) { echo 'al-review'; }else{ echo 'l-review'; } ?>" data-id="<?=$new_blog[0]['id'];?>" style="cursor: pointer;"><i class="mr-5 ml-5 fa fa-thumbs-up"></i> <?php if(!empty($likes)) { echo 'Liked'; }else{ echo 'Like'; } ?></span>
                            ...
                            <form action="<?=ROOT_URL; ?>models/add_comment_front.php" class="comment-form row mt-50" method="post">
                                ...
                                <input type="hidden" name="nblog_id" value="<?=$new_blog[0]['id']; ?>">
                                <div class="col-xs-12">

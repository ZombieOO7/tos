...
<table width="100%">
    ...
    <div id="customer">
        <table id="meta">
            ...
            <tr>
                ...
                <td><?=date('d-m-Y',strtotime($results[0]['qt_date'])); ?></td>
            </tr>
            <tr>

                <td class="meta-head">Expiry Date</td>
                <td><?=date('d-m-Y',strtotime($results[0]['qt_expdate'])); ?></td>
            </tr>
            <tr>
                <td class="meta-head">Grand Total</td>
                <td>
                    <div class="due">
                        <?=str_replace('Rs', '₹', $formatter->formatCurrency($results[0]['qt_grandtotal'], 'INR'));?>
                    </div>
                </td>
            </tr>
        </table>
    ...
    <?php if($results[0]['qt_discussion']!== ''){ ?>
        <hr>
        <div>
            <?php echo $results[0]['qt_discussion']; ?>
        </div>
        <hr>
    <?php } ?>
    ...
    <table id="items">
        ...
        <tr>
            ...
            <td class="total-value"><div id="subtotal"><?php echo str_replace( 'Rs', '₹', $formatter->formatCurrency($results[0]['qt_subtotal'], 'INR')); ?></div></td>
        </tr>
        <?php
        if(($results[0]['qt_discountamt']) !=='0'){ ?>
            <tr>
                <td class="blank"> </td>
                <td class="blank"> </td>
                <td class="total-line">Discount <?=strpos($results[0]['qt_disctype'], '%') ? "(".$results[0]['qt_disctype'].")":'' ;?></td>
                <td class="total-value"><div id="subtotal"><?php echo str_replace( 'Rs', '₹', $formatter->formatCurrency($results[0]['qt_discountamt'], 'INR')); ?></div></td>
            </tr>
        <?php
        }
        ?>
       
        <?php $gst_states=json_decode($results[0]['qt_states']); if($gst_states[0]!==$gst_states[1]) { ?>
            <tr>
               	<td class="blank"> </td>
                <td class="blank"> </td>
                <td class="total-line">IGST (<?=$results[0]['qt_taxrate'];?>)</td>
                <td class="total-value"><div id="total">
                    <?=str_replace( 'Rs', '₹', $formatter->formatCurrency($results[0]['qt_igst'], 'INR') );?></div>
                </td>
            </tr>
        <?php } else{ if($results[0]['qt_taxrate']!=="0%"){ 
            $csgst_p=str_replace("%", "", $results[0]['qt_taxrate']);
        ?>
            <tr>
               	<td class="blank"> </td>
                <td class="blank"> </td>
                <td class="total-line">CGST (<?=($csgst_p/2)."%";?>)</td>
                <td class="total-value"><div id="total">
                    <?=str_replace( 'Rs', '₹', $formatter->formatCurrency($results[0]['qt_cgst'], 'INR') );?></div>
                </td>
            </tr>
             <tr>
               	<td class="blank"> </td>
                <td class="blank"> </td>
                <td class="total-line">SGST (<?=($csgst_p/2)."%";?>)</td>
                <td class="total-value"><div id="total">
                    <?=str_replace( 'Rs', '₹', $formatter->formatCurrency($results[0]['qt_sgst'], 'INR') );?></div>
                 </td>
            </tr>
        <?php }} ?>
            ...
            <tr>
                ...
                <td class="total-value balance"><div class="due"><?=str_replace( 'Rs', '₹', $formatter->formatCurrency($results[0]['qt_grandtotal'], 'INR') ); ?></div></td>
            </tr>
    ...
    <table id="items">
            <tr>
                <td class="balance" style="text-align: center; text-transform: uppercase">
                    <?php
                        $number = $results[0]['qt_grandtotal'];
                           $no = round($number);
    ...
    <table>
        <tbody>
            <tr>
                <td> 
                    <?php
                        if(!empty($results[0]['qt_notes']))
                        {
                            echo $results[0]['qt_notes'];  
                        }
                    ?>
                </td>
            </tr>

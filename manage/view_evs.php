...
if(isset($_SESSION['user_id']))
{
	...
	{
		$sql="SELECT * FROM evs ORDER BY id DESC;";
					...
					<div class="page-header page-header-default">
						<div class="page-header-content">
							<div class="page-title">
								<h2 class="no-margin text-light">ALL EV's</h2>
							</div>
						</div>
					</div>
					<div class="content">
									...
									<div class="panel-body">
										<table class="table datatable-basic-uo table-striped table-bordered">
											...
											<tbody>
												...
												<tr>
													<td><?=date("d-m-Y",strtotime($result['e_date'])); ?></td>
													<td><?=$result['e_name']; ?></td>
													<td><?=$result['e_author']; ?></td>
													<td>
													...
													<td><?=str_word_count(strip_tags($result['e_content'])); ?></td>
													...
													<td>
														<ul class="icons-list">
															<li class="text-primary-600">
																<a href="edit_ev.php?id=<?=$result['id'];?>" data-popup="tooltip" data-original-title="Edit"><i class="icon-pencil6"></i></a>
															</li>
															<li class="text-primary-600">
																<?php if($result['publish']=="1"){ ?>
																	<a href="<?=ROOT_URL; ?>models/publish_evs.php?id=<?=$result['id'];?>&st=0" data-popup="tooltip" data-original-title="Unpublish"><i class="icon-cross"></i></a>
																<?php }elseif($result['publish']=="0"){ ?>	
																	<a href="<?=ROOT_URL; ?>models/publish_evs.php?id=<?=$result['id'];?>&st=1" data-popup="tooltip" data-original-title="Publish"><i class="icon-check"></i></a>
																<?php }else{} ?>
															</li>
															<li class="text-primary-600">
																<a href="<?=ROOT_URL_FRONT."evs/".$result['url_id'].".html";?>" target="_blank" data-popup="tooltip" data-original-title="Preview"><i class="icon-eye"></i></a>
															</li>
															<li class="text-primary-600">
        ...
        {	
        	...
        	function(isConfirm)
        	{
        		if (isConfirm) 
        		{
        			$.ajax({
			            type: 'POST',
			            url: '<?=ROOT_URL;?>models/delete_evs.php',
			            data:{'d_id':d_id},
			            /*dataType:'json',*/

...
{	
	...
	try
	{	
		$sql="UPDATE columns SET $up_data WHERE id='$id';";
		$q=$conn->prepare($sql);
		if($q->execute($data))
		{
			if($st=="1")
			{
				$sqlchk="SELECT * FROM columns WHERE id='$id';";
                ...
                $searchfor = "https://www.turnofspeed.in/column/".$chk_res[0]['url_id'].".html";
				...
				if(!preg_match_all($pattern, $contents, $matches)){
                    ...
                    if($url_count=="1000" || $url_count=="0")
                    {
                        ...
                        "               <news:title>".htmlspecialchars($chk_res[0]['b_name'])."</news:title>\n".
                        "           </news:news>\n".
                        "       </url>\n".
                        "</urlset>");
                        fclose ($pf);
                ...
                $sqlup="UPDATE columns SET is_index='$status_code' WHERE id='$id';";
				...
				$_SESSION['s']="Column Published Successfully !!!";
			}
			else
			{
				$_SESSION['s']="Column Un-published Successfully !!!";
			}
			header('Location:'.ROOT_URL.'view_columns.php');
		}
		else
		{
			$_SESSION['e']="Failed To Published / Un-published Column, Please Try Again !!!";

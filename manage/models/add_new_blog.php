...
if(isset($_POST["submit"])) 
{
	...
	try
	{	
		$sql = "INSERT INTO articles ($insertCols) VALUES ($cols);";
		$q= $conn->prepare($sql);
		if($q->execute($data))	
		{
			$sqls = "SELECT id,b_content FROM articles WHERE created_by='$user_id' ORDER BY id DESC LIMIT 1;";
			$qs= $conn->query($sqls);
			...
			if(isset($_FILES['cover_file']['name']) &&  !empty($_FILES['cover_file']['name']) )
			{
				...
				$path=ROOT_PATH."uploads/new_blog/";
				...
				{
					$sqlup="UPDATE articles SET cover_photo='$final_profile' WHERE id='$blogid';";
					$qup= $conn->prepare($sqlup);
					$qup->execute();

					$_SESSION['s']="Blog Added Successfully !!!";
					header('Location:'.ROOT_URL.'view_new_blog.php');
				}
			...
			if(isset($_FILES['featured_file']['name']) &&  !empty($_FILES['featured_file']['name']) && $is_featured=="on")
			{
				...
				{
					$sqlupf="UPDATE articles SET featured_photo='$profile' WHERE id='$blogid';";
			...
			$update_content="UPDATE articles SET b_content='$content' WHERE id='$blogid';";

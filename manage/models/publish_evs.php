...
{	
	...
	try
	{	
		$sql="UPDATE evs SET $up_data WHERE id='$id';";
		$q=$conn->prepare($sql);
		if($q->execute($data))
		{
			if($st=="1")
			{
				$sqlchk="SELECT * FROM evs WHERE id='$id';";
                ...
                $searchfor = "https://www.turnofspeed.in/evs/".$chk_res[0]['url_id'].".html";
				...
				if(!preg_match_all($pattern, $contents, $matches)){
                    ...
                    if($url_count=="1000" || $url_count=="0")
                    {
                        ...
                        "               <news:title>".htmlspecialchars($chk_res[0]['e_name'])."</news:title>\n".
                        "           </news:news>\n".
                        "       </url>\n".
                        "</urlset>");
                        fclose ($pf);
                ...
                $sqlup="UPDATE evs SET is_index='$status_code' WHERE id='$id';";
				...
				$_SESSION['s']="EV Published Successfully !!!";
			}
			else
			{
				$_SESSION['s']="EV Un-published Successfully !!!";
			}
			header('Location:'.ROOT_URL.'view_evs.php');
		}
		else
		{
			$_SESSION['e']="Failed To Published / Un-published EV, Please Try Again !!!";

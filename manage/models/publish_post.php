...
{	
	...
	try
	{	
		$sql="UPDATE reviews SET $up_data WHERE id='$id';";
		$q=$conn->prepare($sql);
		if($q->execute($data))
		{
			if($st=="1")
			{
				$sqlchk="SELECT * FROM reviews WHERE id='$id';";
                ...
                $searchfor = "https://www.turnofspeed.in/review/".$chk_res[0]['url_id'].".html";
				...
				if(!preg_match_all($pattern, $contents, $matches)){
                    ...
                    if($url_count=="1000" || $url_count=="0")
                    {
                        ...
                        "               <news:title>".htmlspecialchars($chk_res[0]['p_name'])."</news:title>\n".
                        "           </news:news>\n".
                        "       </url>\n".
                        "</urlset>");
                        fclose ($pf);
                ...
                $sqlup="UPDATE reviews SET is_index='$status_code' WHERE id='$id';";
				...
				$_SESSION['s']="Review Published Successfully !!!";
			}
			else
			{
				$_SESSION['s']="Review Un-published Successfully !!!";
			}
			header('Location:'.ROOT_URL.'view_posts.php');
		}
		else
		{
			$_SESSION['e']="Failed To Published / Un-published Review, Please Try Again !!!";

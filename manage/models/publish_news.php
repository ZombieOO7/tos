...
{	
	...
	try
	{	
		$sql="UPDATE news SET $up_data WHERE id='$id';";
		$q=$conn->prepare($sql);
		if($q->execute($data))
		{
			if($st=="1")
			{
				$sqlchk="SELECT * FROM news WHERE id='$id';";
                ...
                $searchfor = "https://www.turnofspeed.in/news/".$chk_res[0]['url_id'].".html";
				...
				if(!preg_match_all($pattern, $contents, $matches)){
                    ...
                    if($url_count=="1000" || $url_count=="0")
                    {
                        ...
                        "               <news:title>".htmlspecialchars($chk_res[0]['n_name'])."</news:title>\n".
                        "           </news:news>\n".
                        "       </url>\n".
                        "</urlset>");
                        fclose ($pf);
                ...
                $sqlup="UPDATE news SET is_index='$status_code' WHERE id='$id';";
				...
				$_SESSION['s']="News Published Successfully !!!";
			}
			else
			{
				$_SESSION['s']="News Un-published Successfully !!!";
			}
			header('Location:'.ROOT_URL.'view_news.php');
		}
		else
		{
			$_SESSION['e']="Failed To Published / Un-published News, Please Try Again !!!";

...
{	
	...
	{	
		...
		$sqls="SELECT cover_photo FROM columns WHERE id='$d_id';";
		$qs= $conn->query($sqls);
		...
		if($c_flag==1)
		{
		  	$sql="DELETE FROM columns WHERE id='$d_id';";
			$q= $conn->prepare($sql);
			if($q->execute())
			{
				$_SESSION['s']="Column Deleted Successfully !!!";
			}
			else
			{
				$_SESSION['e']="Failed To Delete Column !!!";
			}
		}
		else
		{

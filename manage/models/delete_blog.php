...
{	
	...
	{	
		...
		$sqls="SELECT cover_photo,featured_photo FROM features WHERE id='$d_id';";
		$qs= $conn->query($sqls);
		...
		{
		  	$sql="DELETE FROM features WHERE id='$d_id';";
			$q= $conn->prepare($sql);
			if($q->execute())
			{
				$_SESSION['s']="Feature Deleted Successfully !!!";
			}
			else
			{
				$_SESSION['e']="Failed To Delete Feature !!!";
			}
		}
		else
		{

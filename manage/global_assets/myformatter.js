...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // ===== Driver Input Formatter Start =======
    //Driver Adhaar Card
    $('[name="dr_adhno"]').formatter({
        pattern: '{{9999}}-{{9999}}-{{9999}}'
    });
	
	//Driver PAN Card
	$('[name="dr_panno"]').formatter({
        pattern: '{{aaaaa9999a}}',
    });
	
	//Driver Contact Number
    $('[name="cust_phone"]').formatter({
        pattern: '+{{99}}-{{9999999999}}'
    });
	// ===== Driver Input Formatter End =======
	
	// ===== Vehicle Input Formatter Start =======
	//Vehicle Lorry No
    $('[name="lorry_no"]').formatter({
        pattern: '{{aa99}}-{{aa}}-{{9999}}'
    });

    //Vehicle  No
    $('.vehicle_no').formatter({
        pattern: '{{aa99}}-{{aa}}-{{9999}}'
    });
    // ===== Vehicle Input Formatter End =======

	// ===== Add Organization Start =======
	//Org GST No
    $('[name="org_gstno"]').formatter({
        pattern: '{{aaaaa9999a9a*}}',
    });
	
	//Org CIN No
    $('[name="org_cin"]').formatter({
        pattern: '{{a99999aa9999aaa999999}}',
    });
	
	//Org LLPIN No
    $('[name="org_llpin"]').formatter({
        pattern: '{{aaa}}-{{9999}}',
    });
	
	//Org Primary Contact Number
    $('[name="org_pphone"]').formatter({
        pattern: '+91-{{9999999999}}'
    });
	
	//Org Secondry Contact Number
    $('[name="org_sphone"]').formatter({
        pattern: '+91-{{9999999999}}'
    });
	// ===== Add Organization End =======
	
    /*add lead form*/
    $('[name="phone_no"]').formatter({
        pattern: '+91-{{9999999999}}'
    });

     $('[name="s_phone"]').formatter({
        pattern: '+91-{{9999999999}}'
    });
      
    /*add lead from end*/
	
     $('[name="contact-no"]').formatter({
        pattern: '+91-{{9999999999}}'
    });
    // Credit card
...
// add customer/company/vendor

 $('[name="com_gstno"]').formatter({
        pattern: '{{aaaaa9999a9a*}}',
    });

 $('[name="com_phone"]').formatter({
        pattern: '+91-{{9999999999}}'
    });

 $('[name="p_phone"]').formatter({
        pattern: '+91-{{9999999999}}'
    });
...
// add employee
  ...
  $('[name="e_phone"]').formatter({
        pattern: '+91-{{9999999999}}'
    });

  $('[name="e_pin"]').formatter({
        pattern: '{{999999}}'
    });

  // add warehouse

  $('[name="wh_phone"]').formatter({
        pattern: '+91-{{9999999999}}'
    });

  $('[name="wh_pin"]').formatter({
        pattern: '{{999999}}'
    });

//full load trip
 $('[name="rent_drphone"]').formatter({
        pattern: '+91-{{9999999999}}'
    });


//add account
  $('[name="cp_phone"]').formatter({
        pattern: '+91-{{9999999999}}'
    });

  //add user
  $('[name="u_phone"]').formatter({
        pattern: '+91-{{9999999999}}'
    });

...
if(isset($_SESSION['user_id']))
{
	...
	{
		$sql="SELECT n.*,nc.name as nc_name FROM news n LEFT JOIN news_category nc ON n.n_cat_id=nc.id ORDER BY n.id DESC;";
					...
					<div class="page-header page-header-default">
						<div class="page-header-content">
							<div class="page-title">
								<h2 class="no-margin text-light">ALL NEWS</h2>
							</div>
						</div>
					</div>
					<div class="content">
										...
										<table class="table datatable-basic-uo table-striped table-bordered">
											<thead>
												<tr>	
													...
													<th>News Heading</th>
													<th>Author</th>
													<th width="80px">Comment</th>
													<th width="80px">Words</th>
													<th width="80px">Views</th>
													<th width="80px">Likes</th>
													<th width="88px">Published</th>
													<th>Index</th>
											...
											<tbody>
												...
												<tr>
													<td><?=date("d-m-Y",strtotime($result['n_date'])); ?></td>
													<td><?=$result['nc_name']; ?></td>
													<td><?=$result['n_name']; ?></td>
													<td><?=$result['n_author']; ?></td>
													<td>
													...
													<td><?=str_word_count(strip_tags($result['n_content'])); ?></td>
													...
													<td>
														<ul class="icons-list">
															<li class="text-primary-600">
																<a href="edit_news.php?id=<?=$result['id'];?>" data-popup="tooltip" data-original-title="Edit"><i class="icon-pencil6"></i></a>
															</li>
															<li class="text-primary-600">
																<?php if($result['publish']=="1"){ ?>
																	<a href="<?=ROOT_URL; ?>models/publish_news.php?id=<?=$result['id'];?>&st=0" data-popup="tooltip" data-original-title="Unpublish"><i class="icon-cross"></i></a>
																<?php }elseif($result['publish']=="0"){ ?>	
																	<a href="<?=ROOT_URL; ?>models/publish_news.php?id=<?=$result['id'];?>&st=1" data-popup="tooltip" data-original-title="Publish"><i class="icon-check"></i></a>
																<?php }else{} ?>
															</li>
															<li class="text-primary-600">
																<a href="<?=ROOT_URL_FRONT."news/".$result['url_id'].".html";?>" target="_blank" data-popup="tooltip" data-original-title="Preview"><i class="icon-eye"></i></a>
															</li>
															<li class="text-primary-600">
        ...
        {	
        	...
        	function(isConfirm)
        	{
        		if (isConfirm) 
        		{
        			$.ajax({
			            type: 'POST',
			            url: '<?=ROOT_URL;?>models/delete_news.php',
			            data:{'d_id':d_id},
			            /*dataType:'json',*/

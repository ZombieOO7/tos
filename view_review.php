...
{
	extract($_GET);
	if(strpos($share_link, "view_review.php?p_id=")!==false)
    {
        header('HTTP/1.1 301 Moved Permanently');
        header('Location:'.ROOT_URL_FRONT.'review/'.$p_id.'.html');
        exit();
    }

    if(array_key_exists("user_role", $_SESSION))
    {
        $sqlp="SELECT * FROM reviews WHERE url_id='$p_id';";
        $qp= $conn->query($sqlp);
        $post_ones=$qp->fetchAll(PDO::FETCH_ASSOC);
    }
    else
    {
    	$sqlp="SELECT * FROM reviews WHERE publish='1' AND url_id='$p_id';";
    	$qp= $conn->query($sqlp);
    	$post_ones=$qp->fetchAll(PDO::FETCH_ASSOC);

        $sqlup="UPDATE reviews SET view_count=(view_count+1) WHERE publish='1' AND id='".$post_ones[0]['id']."';";
        $qup= $conn->prepare($sqlup);
        $qup->execute();
    }

    $sqlc="SELECT c.*,ct.cust_name FROM comments c LEFT JOIN customers ct ON  c.cust_id=ct.id WHERE c.post_id='".$post_ones[0]['id']."';";
    $qc= $conn->query($sqlc);
...
if (!empty($_COOKIE["cust_hash"])) {
    $likes="SELECT * FROM like_section WHERE cust_hash='".$_COOKIE['cust_hash']."' AND review_id='".$post_ones[0]['id']."';";
    $likes=$conn->query($likes);
    $likes=$likes->fetchAll(PDO::FETCH_ASSOC);
}

$page_name=$post_ones[0]['p_name'];
$desc=$post_ones[0]['p_subhead'];
...
<script type="application/ld+json">
{
    ...
    "mainEntityOfPage":
    {
        ...
        "headline":"<?=$post_ones[0]['p_name']; ?>",
        "datePublished":"<?=date("c",strtotime($post_ones[0]['p_date'])); ?>",
        "dateModified":"<?=date("c",strtotime($post_ones[0]['updated_at'])); ?>",
        "articleBody":"<?=trim(preg_replace('/\s\s+/', ' ', str_replace("\n", " ", strip_tags($post_ones[0]['p_content']))));?>",
        "keywords":"<?=$post_ones[0]['tags'];?>",
        "url":"<?=(isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";?>",
        "description":"<?php if (!empty($post_ones[0]['p_subhead'])) { echo $post_ones[0]['p_subhead']; }else{ echo trim(preg_replace('/\s\s+/', ' ', str_replace("\n", " ", strip_tags($post_ones[0]['p_content'])))); } ?>",
        "author":
        [
            {
                "@type":"Person",
                "name":"<?php if (!empty($post_ones[0]['p_author'])) { echo $post_ones[0]['p_author']; }else{ echo "Rachna Tyagi"; } ?>"
            },
            {
                "@type":"Thing",
                "name":"Turn of Speed",
        ...
        [
            {
                "@type":"ImageObject",
                "url":"<?php $cp_images=json_decode($post_ones[0]['cover_photo'],true); echo ROOT_URL.$cp_images[0]; ?>"
            }
        ]
    }
}
...
<?php
if($post_ones[0]['is_cover_story']=='1'){ ?>
    <style>
        html::before {background-image: url('../images/tos_grey.jpg') !important;opacity: 1 !important;}
        .hentry-single{background-color: rgba(0,0,0,0.4) !important;}
			...
			<div class="container">
				<div class="row hentry-single">
                   <div class="col-md-12 col-sm-12"> 
                        <h3 class="hentry-title"><?=$post_ones[0]['p_name']; ?></h3>
                        <span class="sep block mt-10 mb-10"></span>
                        <h6 class="hentry-title"><i><?=$post_ones[0]['p_subhead']; ?></i></h6>
                        <br>
    					<div class="col-xs-12 col-md-8">
        					<article>
                                <?php if(!empty($post_ones)){ foreach($post_ones as $post_one){ ?>
                                   <div class="hentry-meta flex flex-middle disable-flex-xs fw700 mt-15">
                                        <span class="posted-on mr-10"><i class="mr-5 fa fa-calendar-check-o"></i> <?=date("M d, Y",strtotime($post_one['p_date'])); ?></span>
                                    <?php if($orgData[0]['post_by']=="1"){ ?>   
                                        <?php if(!empty($post_one['p_author'])){ ?>
                                        <span class="author mr-10"><i class="mr-5 fa fa-user"></i><?=$post_one['p_author']; ?></span>
                                         <?php } ?>
                                    <?php } ?>
                                    ...
                                    <?php } ?>
                                        <span class="comments float l-review <?php if(!empty($likes)) { echo 'al-review'; }else{ echo 'l-review'; } ?>" data-id="<?=$post_ones[0]['id'];?>" style="cursor: pointer;"><i class="mr-5 ml-5 fa fa-thumbs-up"></i> <?php if(!empty($likes)) { echo 'Liked'; }else{ echo 'Like'; } ?></span>
                                    </div> <!-- end .hentry-meta -->

                                    <div class="hentry-content mt-20">
                                        <?php $cp_images=[]; $cp_images=json_decode($post_one['cover_photo'],true); ?>

                                    <?php  if(count($cp_images)==1){ foreach ($cp_images as $cp_image) { ?> 
                                            <figure class="media mb-10">
                                                <img src="<?=ROOT_URL.$cp_image; ?>" class="img-responsive">
                                                <?php if(!empty($post_one['p_graphics'])){ ?>
                                                    <div class="photo-credit pic"> GRAPHICS : <?=$post_one['p_graphics'];?></div>
                                                <?php } ?>
                                                <?php if(!empty($post_one['p_illustration'])){ ?>
                                                    <div class="photo-credit ill"> ILLUSTRATION : <?=$post_one['p_illustration'];?></div>
                                                <?php } ?>
                                                <?php if(!empty($post_one['p_photo_credit'])){ ?>
                                                    <div class="photo-credit gra"> PIC : <?=$post_one['p_photo_credit'];?></div>
                                                <?php } ?>
                                            </figure>
                                        <?php } ?>
                                        <div class="pull-left">
                                            <?php if($orgData[0]['post_by']=="1"){ ?>  
                                                <?php if(!empty($post_one['p_author'])){ ?>
                                                    <p><i class="f700">By</i> : <span class="by-line"><?=$post_one['p_author'];?></p>
                                                <?php } ?>
                                            <?php } ?>
                                        </div><br> 
                                    <?php }else{ ?> 
                                        <div class="post-slider ms-skin-default">
                                            ...
                                            <div class="ms-slide slide-1" data-delay="3"> 
                                                ...
                                                <h3 class="ms-layer text17" style="top:450px; right:0px; width:500px;" data-type="text" data-delay="0" data-effect="scale(1,1)">
                                                    <?php if(!empty($post_one['p_photo_credit'])){ ?>
                                                        <p class="photo-credit">PIC : <?=$post_one['p_photo_credit'];?></p>
                                                    <?php } ?>
                                                </h3>
                                            </div>
                                            <?php } ?>
                                    ...
                                    <p align="justify">
                                    <?php
									$content=str_replace('font-family:Calibri','',str_replace("REPLACE_URL",ROOT_URL,$post_one['p_content']));
                                    if(!empty($post_one['p_location']))
                                    { 
                                        echo $post_one['p_location'].' : '.$content; 
                                    }
                                    else
                                    { 
                                        echo $content; 
                                    ...
                                    <div class="hentry-footer row flex flex-middle disable-flex-xs mt-30">
                                        <?php $all_tags=[]; $all_tags=explode(",", $post_one['tags']); ?>
                            ...
                            <form action="<?=ROOT_URL; ?>models/add_comment_front.php" class="comment-form row mt-50" method="post">
                                <div class="col-xs-12 mb-20">
                                    ...
                                    <input type="hidden" name="post_id" value="<?=$post_ones[0]['id']; ?>">
                                    <div class="col-xs-12">
